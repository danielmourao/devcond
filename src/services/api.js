...
export default {
  ...
  loginValidate: async (cpf, password) => {
  ...
  likeWallPost: async (id) => {
    let token = await AsyncStorage.getItem('token');
    let json = request('post', `/wall/${id}/like`, {}, token);
    return json;
  },
  getDocs : async () => {
    let token = await AsyncStorage.getItem('token');
    let json = request('get', '/docs', {}, token);
    return json;
  },
  getBillets : async () => {
    let token = await AsyncStorage.getItem('token');
    ...
    let json = request('get', '/billets', {property: property.id}, token);
    return json;
  },
  getWarnings: async () => {
    let token = await AsyncStorage.getItem('token');
    let property = await AsyncStorage.getItem('property');
    let json = request('get', '/warnings', {property: property}, token);
    console.log(json);
    return json;
  },
